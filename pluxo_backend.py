...
# Canonical stock inventory document used by document import + sold-line removal sync.
...
def _cross_process_merge_disk_into_state(
    disk: dict[str, Any], *, merge_stock: bool = True
) -> None:
    """
    ...
    # For explicit stock replacement/clear flows we skip this merge.
    dsk_stock = disk.get("stock")
    if merge_stock and isinstance(dsk_stock, list):
...
def save_state(*, merge_stock_from_disk: bool = True) -> None:
    """
    ...
    merge_stock_from_disk=False is used for explicit stock replacement/clear
    operations where disk-side stock should NOT be unioned back in.
    """
    DATA_DIR.mkdir(parents=True, exist_ok=True)
...
def _stock_looks_phone_token(token: str) -> bool:
    t = (token or "").strip()
    if not t or "@" in t:
        return False
    # Phone tokens should not contain alphabetic characters.
...
def _stock_looks_zip_token(token: str) -> bool:
    t = (token or "").strip()
    if not t:
        return False
    # Supports 5, 9, and postal-like numeric-hyphen forms.
    return bool(re.fullmatch(r"\d{4,10}(?:-\d{2,6})?", t))


def _stock_looks_country_token(token: str) -> bool:
    t = (token or "").strip().upper()
    if not t:
        return False
    if t in FRONTEND_COUNTRY_PRESETS:
        return True
    return t in {"US", "USA", "UNITED STATES", "UNITED STATES OF AMERICA"}


def _stock_has_phone_first_country_zip(parts: list[str]) -> bool:
    """
    Detect variant:
      PAN|MM|YY|CVV|Name|Phone|Street|City|State|Country|ZIP|Brand|...
    """
    if _stock_pipe_layout(parts) != "mm_yy_split" or len(parts) < 12:
        return False
    return (
        _stock_looks_phone_token(parts[5])
        and _stock_looks_country_token(parts[9])
        and _stock_looks_zip_token(parts[10])
    )
...
def _stock_field_indices(parts: list[str]) -> dict[str, Any]:
    ...
    if _stock_has_phone_first_country_zip(parts):
        # PAN|MM|YY|CVV|Name|Phone|Street|City|State|Country|ZIP|Brand|Type|Level|Issuer|…
        return {
            "layout": "mm_yy_phone_first",
            "name": 4,
            "street": 6,
            "city": 7,
            "state": 8,
...
def _commit_stock_cards_unlocked(
    ...
    *,
    ...
    if not cards:
        return 0
    sold_pans = _load_sold_card_pans()
    ...
    to_add: list[str] = []
    for card in cards:
        if _is_card_line_already_sold(card, sold_pans):
            skipped_sold += 1
            continue
        to_add.append(card)
    for card in to_add:
    ...
    log = log_line
    if skipped_sold:
        log = f"{log_line} skipped_already_sold={skipped_sold}"
    _action_log_unlocked(log, uid=uid)
    append_cards_to_shop_stock_document(base_sel, to_add)
    save_state()
    return added
...
def _card_pan_from_payload(payload: str) -> str:
    p = (payload or "").strip()
...
def _load_sold_card_pans() -> frozenset[str]:
    """PANs already sold (from data/sold.txt) — used to block re-import / re-list."""
    if not SOLD_STOCK_DOCUMENT_PATH.is_file():
        return frozenset()
    try:
        raw = SOLD_STOCK_DOCUMENT_PATH.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return frozenset()
    pans: set[str] = set()
    for ln in raw.splitlines():
        pay = _document_line_card_payload(ln) or ln.strip()
        if not pay or "|" not in pay:
            continue
        pan = _card_pan_from_payload(pay)
        if pan:
            pans.add(pan)
    return frozenset(pans)


def _is_card_line_already_sold(card_line: str, sold_pans: frozenset[str]) -> bool:
    if not sold_pans:
        return False
    pan = _card_pan_from_payload((card_line or "").strip())
    return bool(pan and pan in sold_pans)


def append_sold_lines_to_sold_document(sold_full_infos: list[str]) -> None:
    """Archive sold card lines so they are not re-imported into shop stock."""
    if not sold_full_infos:
        return
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        parts: list[str] = []
        for sold in sold_full_infos:
            ...
            if t:
                parts.append(t + "\n")
        if parts:
            with SOLD_STOCK_DOCUMENT_PATH.open("a", encoding="utf-8") as f:
                f.write("".join(parts))
    except OSError:
        pass
...
def finalize_sold_card_sales(sold_full_infos: list[str]) -> None:
    """Move sold cards off shop stock: archive to sold.txt and remove from shop_stock.txt."""
    if not sold_full_infos:
        return
    append_sold_lines_to_sold_document(sold_full_infos)
    remove_sold_lines_from_shop_stock_document(sold_full_infos)
...
def api_admin_stock_document_import():
    ...
    with state_lock:
        ...
        if sold_pans:
            ...
            for base_sel, card in entries:
                if _is_card_line_already_sold(card, sold_pans):
                    ...
                    filtered.append((base_sel, card))
            entries = filtered
        if not entries:
            return jsonify(
                {
                    "ok": False,
                    "error": "All card lines in this file were already sold (see data/sold.txt).",
                }
            ), 400
        ...
        if skipped_sold:
            log_import += f" skipped_already_sold={skipped_sold}"
        _action_log_unlocked(log_import, uid=None)
        save_state(merge_stock_from_disk=False)

    out: dict[str, Any] = {"ok": True, "imported": len(entries), "price": price, "country": country}
    if skipped_sold:
...
def api_topup_submit():
    ...
    # Pending file first so Telegram can resolve before state.json flush finishes.
    _write_topup_pending_file(row)

    def _persist_topup_submit() -> None:
        try:
            # Top-up only touches crypto_topups; skip stock union (faster + avoids stale disk stock merge).
            save_state(merge_stock_from_disk=False)
        except Exception as exc:
            print(f"[topup] persist_submit {pid}: {exc!r}", flush=True)

    threading.Thread(
        target=_persist_topup_submit,
        daemon=True,
        name=f"topup-persist-{pid[:8]}",
    ).start()
    threading.Thread(
...
def _same_shop_product_id(cell: Any, want: Any) -> bool:
    """Match shop row id from JSON vs state (handles 5 vs "5")."""
    if cell == want:
        return True
    try:
        return int(cell) == int(want)
    except (TypeError, ValueError):
        return str(cell or "").strip() == str(want or "").strip()
...
def api_checkout():
    ...
    with state_lock:
        ...
        for it in items:
            ...
            row = next((s for s in stock if _same_shop_product_id(s.get("id"), pid)), None)
            if not row:
        ...
        for row in resolved_stock:
            rm = row.get("id")
            stock[:] = [s for s in stock if not _same_shop_product_id(s.get("id"), rm)]
        ...
        if sold_for_doc:
            finalize_sold_card_sales(sold_for_doc)
