...
1. **`pluxo-api-base` pointed at localhost** — when Flask serves HTML and `/api/*` from one Railway URL, leave **`<meta name="pluxo-api-base" content="">`** empty so the site uses **same-origin** (`window.location.origin`).
2. **HTML on a separate static host** (e.g. custom domain CDN) — set **`pluxo-api-base`** (or browser **`localStorage pluxo_api_base`**) to your **Railway HTTPS API base** (`https://…up.railway.app`).
3. **Dev tools** — the script logs **`API URL:`** at startup; **`GET /pluxo-ok`** on that host must work.
